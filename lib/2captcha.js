...
export const solve = async ({ rucaptcha, key, pageurl, sitekey, method = 'hcaptcha' }) => {
...
export const report = async ({ rucaptcha, key, id, report = 'bad' }) => {
  const { body } = await got.get(
...
export const balance = async ({ rucaptcha, key, id, type = 'getbalance' }) => {
  const { body } = await got.get(
